...
help() {
  cat <<'EOF'
Install a binary release of ordwoj hosted on GitHub
...
crate=ordwoj
url=https://github.com/WojakCoinProj/ord-wojakcoin
...
if [ -z "${tag-}" ]; then
  tag=$(
    download https://api.github.com/repos/mvdnbrk/ord-wojakcoin/releases/latest - |
    grep tag_name |
    cut -d'"' -f4
  )
fi
