...
pub struct Settings {
  ...
  pub(crate) wojakcoin_data_dir: Option<PathBuf>,
  pub(crate) wojakcoin_rpc_limit: Option<u32>,
  pub(crate) wojakcoin_rpc_password: Option<String>,
  pub(crate) rpc_url: Option<String>,
  pub(crate) wojakcoin_rpc_username: Option<String>,
...
impl Settings {
  pub fn load(options: Options) -> Result<Self> {
    ...
    for (var, value) in env::vars_os() {
      ...
      if let Some(key) = var.strip_prefix("ORDWOJ_") {
        env.insert(
          key.into(),
  ...
  pub(crate) fn or(self, source: Self) -> Self {
    Self {
      ...
      wojakcoin_data_dir: self.wojakcoin_data_dir.or(source.wojakcoin_data_dir),
      wojakcoin_rpc_limit: self.wojakcoin_rpc_limit.or(source.wojakcoin_rpc_limit),
      wojakcoin_rpc_password: self.wojakcoin_rpc_password.or(source.wojakcoin_rpc_password),
      rpc_url: self.rpc_url.or(source.rpc_url),
      wojakcoin_rpc_username: self.wojakcoin_rpc_username.or(source.wojakcoin_rpc_username),
  ...
  fn from_options(options: Options) -> Self {
    Self {
      ...
      max_savepoints: None,
      wojakcoin_data_dir: options.wojakcoin_data_dir,
      wojakcoin_rpc_limit: None,
      wojakcoin_rpc_password: None,
      rpc_url: options.rpc_url,
      wojakcoin_rpc_username: None,
  ...
  fn from_env(env: BTreeMap<String, String>) -> Result<Self> {
    ...
    let get_chain = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| format!("failed to parse environment variable ORDWOJ_{key} as chain"))
    };
    ...
    let inscriptions = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| {
          format!("failed to parse environment variable ORDWOJ_{key} as inscription list")
        })
    };
    ...
    let get_u16 = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| format!("failed to parse environment variable ORDWOJ_{key} as u16"))
    };
    ...
    let get_u32 = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| format!("failed to parse environment variable ORDWOJ_{key} as u32"))
    };
    ...
    let get_usize = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| format!("failed to parse environment variable ORDWOJ_{key} as usize"))
    };
    ...
    let get_url = |key: &str| {
      env
        ...
        .transpose()
        .with_context(|| format!("failed to parse environment variable ORDWOJ_{key} as URL"))
    };
    ...
    Ok(Self {
      ...
      wojakcoin_data_dir: get_path("WOJAKCOIN_DATA_DIR"),
      wojakcoin_rpc_limit: get_u32("WOJAKCOIN_RPC_LIMIT")?,
      wojakcoin_rpc_password: get_string("WOJAKCOIN_RPC_PASSWORD"),
      rpc_url: get_string("RPC_URL"),
      wojakcoin_rpc_username: get_string("WOJAKCOIN_RPC_USERNAME"),
  ...
  fn or_defaults(self) -> Result<Self> {
    ...
    let wojakcoin_data_dir = match &self.wojakcoin_data_dir {
      Some(wojakcoin_data_dir) => wojakcoin_data_dir.clone(),
      None => {
    ...
    let cookie_file = match self.cookie_file {
      ...
      None => chain.join_with_data_dir(&wojakcoin_data_dir).join(".cookie"),
    };
    ...
    Ok(Self {
      ...
      wojakcoin_data_dir: Some(wojakcoin_data_dir),
      wojakcoin_rpc_limit: Some(self.wojakcoin_rpc_limit.unwrap_or(16)),
      wojakcoin_rpc_password: self.wojakcoin_rpc_password,
      rpc_url: Some(
        self
          .rpc_url
          .clone()
      ...
      wojakcoin_rpc_username: self.wojakcoin_rpc_username,
  ...
  #[allow(dead_code)]
  pub(crate) fn wojakcoin_rpc_limit(&self) -> u32 {
    self.wojakcoin_rpc_limit.unwrap()
  }
  ...
  pub(crate) fn auth(&self) -> Result<Auth> {
    if let (Some(user), Some(pass)) = (&self.wojakcoin_rpc_username, &self.wojakcoin_rpc_password) {
  ...
  pub(crate) fn wojakcoin_rpc_client(&self) -> Result<Client> {
    let rpc_url = self.rpc_url();
    let auth = self.auth()?;

    match &auth {
      Auth::CookieFile(path) => log::info!(
        "Connecting to Wojakcoin Core RPC server at {rpc_url} using cookie file `{}`",
        path.display()
      ),
      Auth::UserPass(user, _) => {
        log::info!("Connecting to Wojakcoin Core RPC server at {rpc_url} as user `{user}`")
      }
      Auth::None => {
        log::info!("Connecting to Wojakcoin Core RPC server at {rpc_url} without authentication")
      }
    }

    let client = Client::new(&rpc_url, auth)
      .with_context(|| format!("failed to connect to Wojakcoin Core RPC at {rpc_url}"))?;
    ...
    let rpc_chain = match chain_str {
      ...
      "signet" => Chain::Signet,
      other => bail!("Wojakcoin RPC server on unknown chain: {other}"),
    };

    let ord_chain = self.chain();

    if rpc_chain != ord_chain {
      bail!("Wojakcoin RPC server is on {rpc_chain} but ordwoj is on {ord_chain}");
    }
    Ok(client)
  }

  // Wojakcoin Core 1.1.0 doesn't support /wallet/<name> HTTP endpoint.
  // Keep for future use when multi-wallet HTTP routing is available.
  #[allow(dead_code)]
  pub(crate) fn wojakcoin_rpc_client_for_wallet(&self, wallet_name: &str) -> Result<Client> {
    let rpc_url = format!("{}/wallet/{}", self.rpc_url(), wallet_name);
    let auth = self.auth()?;

    log::info!("Connecting to Wojakcoin Core RPC server for wallet `{wallet_name}` at {rpc_url}");

    Client::new(&rpc_url, auth)
      .with_context(|| format!("failed to connect to Wojakcoin Core RPC at {rpc_url}"))
  }

  pub(crate) fn wojakcoin_rpc_client_for_wallet_command(&self) -> Result<Client> {
    let client = self.wojakcoin_rpc_client()?;
    ...
    let wojakcoin_version = usize::try_from(
      network_info["version"]
        .as_u64()
        .ok_or_else(|| anyhow!("missing version in getnetworkinfo"))?,
    )
    .unwrap();
    if wojakcoin_version < MIN_VERSION {
      bail!(
        "Wojakcoin Core {}.{}.{}.{} or newer required, current version is {}.{}.{}.{}",
        MIN_VERSION / 1000000,
        MIN_VERSION % 1000000 / 10000,
        MIN_VERSION % 10000 / 100,
        MIN_VERSION % 100,
        wojakcoin_version / 1000000,
        wojakcoin_version % 1000000 / 10000,
        wojakcoin_version % 10000 / 100,
        wojakcoin_version % 100,
      );
    }
...
mod tests {
  ...
  fn use_default_network() {
    ...
    assert_eq!(settings.rpc_url(), "127.0.0.1:20760");
  ...
  fn othernet_cookie_file_path() {
    ...
    assert!(cookie_file.ends_with(if cfg!(target_os = "linux") {
      "/.wojakcoin/signet/.cookie"
    } else if cfg!(windows) {
      r"\Wojakcoin\signet\.cookie"
    } else {
      "/Wojakcoin/signet/.cookie"
    }));
  }
  ...
  #[test]
  fn cookie_file_defaults_to_wojakcoin_data_dir() {
    let cookie_file = parse(&["--wojakcoin-data-dir=foo", "--chain=signet"])
      .cookie_file()
      .unwrap()
      .display()
      .to_string();
  ...
  fn mainnet_data_dir() {
    ...
    assert!(
      data_dir.ends_with(if cfg!(windows) { r"\ordwoj" } else { "/ordwoj" }),
      "{data_dir}"
    );
  }
  ...
  fn network_accepts_aliases() {
    ...
    check_network_alias("main", "ordwoj");
    check_network_alias("mainnet", "ordwoj");
    check_network_alias(
      "regtest",
      if cfg!(windows) {
        r"ordwoj\regtest"
  ...
  fn from_env() {
    ...
    env.insert("WOJAKCOIN_RPC_LIMIT".into(), "10".into());
    ...
    assert_eq!(settings.wojakcoin_rpc_limit, Some(10));
  }
