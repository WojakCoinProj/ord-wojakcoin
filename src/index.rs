...
impl Index {
  ...
  pub(crate) fn get_transaction(&self, txid: Txid) -> Result<Option<Transaction>> {
    if txid == self.genesis_block_coinbase_txid {
      ...
      wojakcoin_rpc::get_raw_transaction(&self.client, &txid).into_option()
    }
  }

  pub(crate) fn get_transaction_blockhash(&self, txid: Txid) -> Result<Option<BlockHash>> {
    Ok(
      wojakcoin_rpc::get_raw_transaction_blockhash(&self.client, &txid)
        .into_option()?
        .flatten(),
    )
  }
  ...
  pub(crate) fn is_transaction_in_active_chain(&self, txid: Txid) -> Result<bool> {
    Ok(
      wojakcoin_rpc::is_transaction_in_active_chain(&self.client, &txid)
        .into_option()?
        .unwrap_or(false),
    )
  }
  ...
  pub(crate) fn get_output_info(&self, outpoint: OutPoint) -> Result<Option<OutputInfo>> {
    ...
    if outpoint == OutPoint::null() {
      ...
      confirmations = wojakcoin_rpc::get_confirmations(&self.client, &outpoint.txid)
        .into_option()?
        .flatten()
        .unwrap_or(0);
      spent = !indexed;
...
mod tests {
  ...
  #[test]
  #[ignore]
  fn missing_inputs_are_fetched_from_wojakcoin_core() {
    for args in [
