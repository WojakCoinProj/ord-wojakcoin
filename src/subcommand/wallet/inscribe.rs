...
pub(crate) struct Inscribe {
  ...
  #[clap(
    long,
    help = "Include JSON traits from <JSON_TRAITS> file as WJK-721 properties.",
    conflicts_with = "batch"
  )]
...
impl Inscribe {
  ...
  pub(crate) fn run(self, wallet: Wallet) -> Result {
    ...
    if fee_rate < min_fee_rate {
      bail!("fee rate must be at least {min_fee_rate} sat/vB (Wojakcoin minimum relay fee)");
    }
    if commit_fee_rate < min_fee_rate {
      bail!("commit fee rate must be at least {min_fee_rate} sat/vB (Wojakcoin minimum relay fee)");
    }
    ...
    if let Some(batch_path) = &self.batch {
      ...
      if self.dry_run {
        ...
        if num_chunks > 1 {
          log::info!("Run 'ordwoj wallet broadcast' or start the server to begin broadcasting.");
        }
      }
    } else {
