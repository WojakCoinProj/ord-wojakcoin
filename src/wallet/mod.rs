...
impl Wallet {
...

    {
      ...
      for change in [false, true] {
        let derivation_path = DerivationPath::master()
          ...
          .child(ChildNumber::Hardened { index: 3434 }) // WOJAKCOIN_COIN_TYPE
  ...
  pub(crate) fn load(
    ...
    let db_path = settings
      ...
      .join("wallet.redb");
    if !db_path.exists() {
      bail!("wallet `{wallet_name}` does not exist, create it first with `ordwoj wallet create`");
    }

    let bitcoin_client = settings.wojakcoin_rpc_client_for_wallet_command()?;
    ...
    if !no_sync {
      ...
      loop {
        let response = ord_client.get(rpc_url.join("/blockcount")?).send()
          .context("wallet failed to retrieve block count from server. Make sure `ordwoj server` is running.")?;
        if !response.status().is_success() {
          bail!(
            "failed to get blockcount from ordwoj server: {}",
            response.status()
          );
        }
    ...
    for outpoint in locked_outpoints {
      ...
      let tx = wojakcoin_rpc::get_raw_transaction(&bitcoin_client, &outpoint.txid)?;
    ...
    if !response.status().is_success() {
      bail!(
        "failed to get outputs from ordwoj server: {}",
        response.status()
      );
    }
    ...
    let (inscriptions, inscription_info) = if !inscription_ids.is_empty() {
      ...
      if !response.status().is_success() {
        bail!(
          "failed to get inscriptions from ordwoj server: {}",
          response.status()
        );
      }
  ...
  pub(crate) fn get_inscription(&self, id: InscriptionId) -> Result<Option<api::Inscription>> {
    ...
    if response.status().is_success() {
      ...
      bail!(
        "failed to get inscription from ordwoj server: {}",
        response.status()
      );
    }
  }
  ...
  pub(crate) fn get_unspent_output_ranges(&self) -> Result<Vec<(OutPoint, Vec<(u128, u128)>)>> {
    if !self._has_sat_index {
      bail!("ordwoj server does not have a sat index");
    }
    Ok(
      self
        .output_info
  ...
  pub(crate) fn get_address(&self, change: bool) -> Result<Address> {
    ...
    let address = {
      ...
      let desc_str = descriptors.get(key)?.ok_or_else(|| {
        anyhow!("wallet contains no descriptors. Was it created with `ordwoj wallet create`?")
      })?;
  ...
  pub(crate) fn get_master_key(&self) -> Result<ExtendedPrivKey> {
    ...
    let seed_bytes = metadata.get("seed")?.ok_or_else(|| {
      anyhow!("wallet contains no seed. Was it created with `ordwoj wallet create`?")
    })?;
